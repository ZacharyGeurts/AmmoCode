...
(function (global) {
  ...
  function bindUi() {
    ...
    $("ac-fly-defield")?.addEventListener("click", async () => {
      try {
        ...
        toast(j.defielded ? "SG/Grok16 defielded" : j.error || "defield failed", !!j.ok);
  ...
  async function refreshSecurityFieldStatus(toastOnClick) {
    try {
      ...
      const def = sg.defield_active || !sg.fielded;
      const label = def ? "defield" : "fielded";
      if (fieldEl) {
        fieldEl.textContent = `SG field: ${label} · ${(sg.signals || []).length} signal(s) · pins ${j.pin_count ?? 0}`;
      }
      if (g16) {
        g16.textContent = def ? "g16 · defield" : "g16 · belt_2_0";
        g16.className = "pill " + (def ? "ok" : "warn");
        g16.title = def ? "Defielded — no field-on-field" : "SG field stack active";
      }
      const sum = $("ac-sec-summary");
      if (sum && toastOnClick) {
