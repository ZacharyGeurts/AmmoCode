...
(function () {
  ...
  function setLanguage(lang, opts) {
    const id = String(lang || "plaintext").toLowerCase();
    state.language = id;
    const label =
      global.AmmoCodeLanguageSuite?.labelFor?.(id) || id;
    if ($("ac-lang")) $("ac-lang").textContent = label;
    const sel = $("ac-lang-select");
    if (sel && sel.value !== id) {
      // keep select in sync when list may not include id yet
      if ([...sel.options].some((o) => o.value === id)) sel.value = id;
    }
    const prof = global.AmmoCodeLanguageSuite?.profileFor?.(id);
    if (prof && !(opts && opts.keepProfile)) {
      state.profile = prof;
      state.settings.profile = prof;
      if ($("ac-profile-label")) $("ac-profile-label").textContent = prof;
      if ($("ac-set-profile")) $("ac-set-profile").value = prof;
      if ($("ac-popover-profile")) $("ac-popover-profile").value = prof;
      global.AmmoCodeG16?.config?.({ beltProfile: prof });
    }
    paint();
    if (opts && opts.status !== false) {
      setStatus(`language · ${label}${prof ? ` · ${prof}` : ""}`);
    }
  }
  ...
  async function discern() {
    ...
    if (!isPagesStatic()) {
      try {
        ...
        if (j.ok && j.language) lang = j.language;
      } catch (_) {}
    }
    if (!lang && global.AmmoCodeG16?.discern) {
      try {
        lang = await global.AmmoCodeG16.discern(state.path, editorText());
      } catch (_) {}
    }
    if (!lang) lang = langFromPath(state.path);
    if (lang) {
      setLanguage(lang, { status: false });
      global.AmmoCodeLanguageSuite?.wire &&
        document.getElementById("ac-lang-select") &&
        global.AmmoCodeLanguageSuite.fillSelect(
          document.getElementById("ac-lang-select"),
          lang
        );
    }
  ...
  async function openPath(path) {
    ...
    setLanguage(j.language || langFromPath(j.path), { status: false });
    global.AmmoCodeLanguageSuite?.fillSelect?.(
      document.getElementById("ac-lang-select"),
      state.language
    );
  ...
  async function g16Run() {
    ...
    const lang = state.language || "plaintext";
    const name = state.path ? state.path.split("/").pop() : "program";
    const pages = isPagesStatic();

    // GitHub Pages + local identical: Run tab sandbox for suite languages
    if (global.AmmoCodeRunSandbox) {
      if (global.AmmoCodeRunSandbox.canSandbox(lang) || pages) {
        global.AmmoCodeRunSandbox.ensureUi?.();
        global.AmmoCodeRunSandbox.run({
          code,
          language: lang,
          name,
          path: state.path,
        ...
        setOutput(
          pages
            ? `[sandbox] ${lang} · Run tab · isolated · same as local suite`
            : `[sandbox] ${lang} · Run tab (identical to GitHub Pages)`,
          true
        );
        setStatus(`running · sandbox · ${lang}`);
        // Still try loopback g16 for systems langs when available
        if (
          !pages &&
          !global.AmmoCodeRunSandbox.canSandbox(lang) &&
          state.path
        ) {
          try {
            const j = await api("g16_run", { path: state.path, language: lang });
            const out = [j.stdout, j.stderr, j.message].filter(Boolean).join("\n");
            if (out) setOutput(out, !!j.ok);
          } catch (_) {}
        }
        return;
      }
    ...
    try {
      if (state.path) {
        j = await api("g16_run", { path: state.path, language: lang });
      } else {
      ...
      setOutput(String(e.message || e), false);
      return;
    }
    const out = [j.stdout, j.stderr, j.message, j.compile?.stderr]
      .filter(Boolean)
      .join("\n");
    setOutput(out || JSON.stringify(j, null, 2), !!j.ok);
  }
  ...
  async function onToolbarAction(id, meta) {
    ...
    switch (id) {
      case "new":
        ...
        setLanguage("plaintext", { status: false });
        global.AmmoCodeLanguageSuite?.fillSelect?.(
          document.getElementById("ac-lang-select"),
          "plaintext"
        );
      ...
      case "discern_lang":
        ...
        if ($("ac-set-autodetect")) $("ac-set-autodetect").checked = true;
        await discern();
        global.AmmoCodeLanguageSuite?.fillSelect?.(
          document.getElementById("ac-lang-select"),
          state.language
  ...
  function bindEditor() {
    ...
    $("ac-file")?.addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        ...
        setLanguage(langFromPath(f.name), { status: false });
        global.AmmoCodeLanguageSuite?.fillSelect?.(
          document.getElementById("ac-lang-select"),
          state.language
        );
  ...
  async function boot() {
    ...
    await loadSettings();
    await loadFiletypes();
    // Language suite — popular first · A–Z · BASIC/QBasic/…
    if (global.AmmoCodeLanguageSuite?.loadSuite) {
      await global.AmmoCodeLanguageSuite.loadSuite();
      global.AmmoCodeLanguageSuite.wire({
        getCurrent: () => state.language,
        onChange: (id) => {
          state.settings.autodetect = false;
          if ($("ac-set-autodetect")) $("ac-set-autodetect").checked = false;
          setLanguage(id);
        },
      });
      setLanguage(state.language || "plaintext", { status: false });
    }
    ...
    if (!pagesStatic) {
      ...
      ping = {
        ok: true,
        version: ping.version || ping.distro_version || "6.2.0",
        grok16: false,
        nondestructive: true,
    ...
    if (g16El) {
      if (pagesStatic) {
        g16El.textContent = "g16 suite · pages";
        g16El.className = "ac-pill ok";
        g16El.title = "Language suite online · clone + Grok16 for compile";
      } else {
        g16El.textContent = ping.grok16 ? "g16 ready" : "g16 offline";
        g16El.className = "ac-pill " + (ping.grok16 ? "ok" : "warn");
      }
    }
    $("ac-version").textContent = `v${ping.version || "6.2.0"}`;
    ...
    const nLang = global.AmmoCodeLanguageSuite?.languages?.length || 0;
    if (ping.extensions) {
      $("ac-ext-count").textContent = nLang
        ? `${ping.extensions} ext · ${nLang} langs`
        : `${ping.extensions} extensions`;
    } else if (nLang) {
      $("ac-ext-count").textContent = `${nLang} languages`;
    }
    const params = new URLSearchParams(location.search);
    const file = params.get("file");
    const langQ = params.get("lang") || params.get("language");
    ...
    setStatus(
      pagesStatic
        ? `AmmoCode suite · popular/A–Z languages · export to save · Grok16 for compile`
        : `ready${saveHint} · suite · Grok16 hard · loopback`
    );
  }
